package
{
   ...
   public class StatsPage_EffectsTab extends IPipBoyTab
   {
      ...
      override public function processProvider(aData:Object) : void
      {
         ...
         var filteredData:Array = aData.EffectsA.filter(ImprovedPipboyStatsConfig.filterEffects);
         filteredData.sortOn(["IconType","Name"]);
         var currType:String = "";
         var accumY:Number = 0;
         for each(info in filteredData)
         {
            if(info.IconType != currType)
            {
               ...
               newWidget.SetWidgetInfo(filteredData);
